/*Link para validação online do teste: https://sqlfiddle.com/postgresql/online-compiler?id=064eb3e8-3ebf-4098-8493-47b2286a25c9*/
...
SELECT id_vendedor as id, nome,salario FROM VENDEDORES order by salario

/*Funcionários com Salário Acima da Média*/
SELECT id_vendedor as id, nome,salario FROM VENDEDORES 
WHERE salario>(SELECT AVG(salario) FROM VENDEDORES) ORDER BY salario DESC



/*Resumo por cliente listando TODOS*/
SELECT CLIENTES.ID_CLIENTE,CLIENTES.RAZAO_SOCIAL,SUM(COALESCE(VALOR_TOTAL,0))TOTAL
FROM PEDIDO
FULL OUTER JOIN CLIENTES ON CLIENTES.ID_CLIENTE=PEDIDO.ID_CLIENTE
GROUP BY CLIENTES.ID_CLIENTE,CLIENTES.RAZAO_SOCIAL
ORDER BY TOTAL DESC


/*Situação por pedido - com descricao do status*/
...
WITH produto_mais_vendido as(
  SELECT 
      ...
      ,SUM(preco_praticado*quantidade)				total_vendido
      ,COUNT(DISTINCT ITENS_PEDIDO.id_pedido)		pedidos
      ,COUNT(DISTINCT PEDIDO.id_cliente)		clientes
      ,DENSE_RANK() OVER(
          PARTITION BY SUM(preco_praticado*quantidade)
          ORDER BY 
        	SUM(quantidade) DESC
        	,SUM(preco_praticado*quantidade) DESC
                                  )					rank
  FROM ITENS_PEDIDO
      LEFT JOIN PEDIDO ON ITENS_PEDIDO.id_pedido=PEDIDO.id_pedido
  GROUP BY id_produto)
 SELECT
 	id_produto
    ,quantidade_vendida
